...
{	//			COORDINATES (X,Y,Z)		/	COLORS			/	TEXTURE MAP	 //
   -0.5f,    0.0f,     0.5f,			0.83f,  0.70f,  0.44f,		0.0f, 0.0f, // Lower left corner (0)
   -0.5f,    0.0f,    -0.5f,			0.83f,  0.70f,  0.44f,		5.0f, 0.0f, // Upper left corner (1)
	0.5f,    0.0f,	  -0.5f,			0.83f,  0.70f,  0.44f,		0.0f, 0.0f, // Upper right corner (2)
    0.5f,    0.0f,	   0.5f,			0.83f,  0.70f,  0.44f,		5.0f, 0.0f, // Inner left corner (3)
	0.0f,    0.8f,	   0.0f,			0.92f,  0.86f,  0.76f,		2.5f, 5.0f, // Inner left corner (3)
};
...
int main()
{
	...
	//----------TEXTURES----------
	TextureClass popCat("Resources/Textures/brick.png", GL_TEXTURE_2D, GL_TEXTURE0, GL_RGBA, GL_UNSIGNED_BYTE);
	...
	while (!glfwWindowShouldClose(window)) //Shouldnt close window
	{
		...
		view = glm::translate(view, glm::vec3(0.0f, -0.5f, -2.0f)); //Move world "camera" back
		projection = glm::perspective(glm::radians(45.0f), (float)(Screen_Width / Screen_Height), 0.1f, 100.0f); //Frustrum view
		...
		//Drawing
		glDrawElements(GL_TRIANGLES, sizeof(indices)/sizeof(int), GL_UNSIGNED_INT, 0); //(Primitave type, num indices, type and index of indices 
